...
def select(user_id,col_use,key):
    ...
    if col_use == "名字":
        print("Trigger!\n")
        postgres_select_query = f"""SELECT * FROM rel_db WHERE user_id=%s AND name=%s"""
        cursor.execute(postgres_select_query,(user_id,key,))
        conn.commit()
    elif col_use == "稱呼":
        postgres_select_query = f"""SELECT * FROM rel_db WHERE user_id=%s AND rel_info=%s"""
        cursor.execute(postgres_select_query,(user_id,key,))
        conn.commit()
    elif key == "全部":
        postgres_select_query = f"""SELECT * FROM rel_db WHERE user_id=%s"""
...
def update(user_id,col_use,key,up_col,upkey):
    ...
    if col_use == "名字":
        if up_col == "名字":
            postgres_update_query = f"""UPDATE rel_db SET name = %s WHERE user_id=%s AND name=%s"""
        elif up_col == "稱呼":
            postgres_update_query = f"""UPDATE rel_db SET rel_info = %s WHERE user_id=%s AND name=%s"""   
    elif col_use == "稱呼":
        if up_col == "名字":
            postgres_update_query = f"""UPDATE rel_db SET name = %s WHERE user_id=%s AND rel_info=%s"""
        elif up_col == "稱呼":
            postgres_update_query = f"""UPDATE rel_db SET rel_info = %s WHERE user_id=%s AND rel_info=%s"""
    cursor.execute(postgres_update_query,(upkey,user_id,key,))
    conn.commit()
...
def delete(user_id,col_use,key):
    ...
    cursor = conn.cursor()
    if col_use == "名字":
        postgres_delect_query = f"""DELETE FROM rel_db WHERE user_id=%s AND name=%s"""
        cursor.execute(postgres_delect_query,(user_id,key,))
    elif col_use == "稱呼":
        postgres_delect_query = f"""DELETE FROM rel_db WHERE user_id=%s AND rel_info=%s"""
        cursor.execute(postgres_delect_query,(user_id,key,))
    elif key == "全部":
        postgres_delect_query = f"""DELETE FROM rel_db WHERE user_id=%s"""
        cursor.execute(postgres_delect_query,(user_id,))
    conn.commit()
    
    count = cursor.rowcount
    cursor.close()
    conn.close()
    return (str(count)+"筆資料已刪除")
